...
/**
 * Open (or create) the IndexedDB database used for recent files storage.
 *
...
/**
 * Retrieve all recent file records from IndexedDB, ordered by lastOpened
 * descending (most recently opened first).
 *
 * @returns An array of RecentFile records.
 */
...
/**
 * Add or update a recent file record in IndexedDB.
 * Enforces a maximum of {@link MAX_RECENT_FILES} entries — the oldest
 * records are removed when the limit is exceeded.
 *
 * @param file - The recent file record to store.
 */
export async function addRecentFile(file: RecentFile): Promise<void> {
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      // Put the new/updated record
      store.put(file);

      // Enforce the maximum count by removing the oldest entries
      const countRequest = store.count();
      countRequest.onsuccess = () => {
        if (countRequest.result > MAX_RECENT_FILES) {
          const excess = countRequest.result - MAX_RECENT_FILES;

          // Index 0 = oldest (lastOpened ascending)
          const index = store.index('lastOpened');
          if (index) {
            // Use getAllKeys on the store sorted by key (name) is not what we want.
            // Instead, collect all records, sort, and delete the oldest.
...
/**
 * Remove a specific recent file record by name.
 *
 * @param name - The unique name of the record to remove.
 */
...
/**
 * Clear all recent file records from IndexedDB.
 */
 ...
 *
 * @returns true if both showOpenFilePicker and showSaveFilePicker are available.
 */
...
/**
 * Generate a standard .pwg filename for a project.
 *
 * @param project - The project to generate a filename for.
 * @returns The filename, e.g. "MyProject.pwg".
 */
...
/**
 * Get the project file extension used by this application.
 *
 * @returns The extension string ".pwg".
 */
